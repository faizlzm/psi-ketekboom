class VirtualMonkey {
    ...
    handleSleepingEvents(event) {
        if (event === 'WAKE_UP') {
            ...
            if (energyGained < 10) {
                this.showMessage('Monyet terbangun terlalu cepat dan masih lelah!', 'warning');
            } else {
        ...
        switch (state) {
            case 'eating':
                this.actionTimeout = setTimeout(() => {
                    this.decreaseHunger(Math.min(this.context.hunger, 40 + Math.floor(Math.random() * 11)));
                    this.increaseLife(5);
            ...
            case 'playing':
                this.actionTimeout = setTimeout(() => {
                    this.increaseMood(Math.min(100 - this.context.mood, 20 + Math.floor(Math.random() * 11)));
            ...
            case 'bathing':
                this.actionTimeout = setTimeout(() => {
                    this.increaseCleanliness(Math.min(100 - this.context.cleanliness, 50 + Math.floor(Math.random() * 21)));
                    this.increaseMood(10);
    ...
    updateMonkeyAppearance() {
        this.monkey.className = 'monkey';
        const validStatesForAnimation = ['eating', 'sleeping', 'playing', 'bathing', 'hungry', 'dead'];
        if (validStatesForAnimation.includes(this.currentState)) {
            this.monkey.classList.add(this.currentState);
        }
        if (this.currentState === 'dead') {
            this.monkey.classList.add('dead');
        } else {
            this.monkey.classList.remove('dead');
        }
    }
    ...
    updateStatusText() {
        ...
        this.statusText.textContent = statusTexts[this.currentState] || 'Kondisi tidak diketahui';
    }
    ...
    updateButtonStates() {
        const isActionState = ['eating', 'sleeping', 'playing', 'bathing'].includes(this.currentState);
        const isDead = this.currentState === 'dead';

        this.buttons.feed.disabled = isActionState || isDead || this.context.hunger === 0;
        this.buttons.sleep.disabled = isActionState || isDead || this.context.energy === 100;
        this.buttons.play.disabled = isActionState || isDead || this.context.energy < 25;
        this.buttons.bathe.disabled = isActionState || isDead || this.context.cleanliness === 100;
    ...
    startDegradation() {
        ...
        this.degradationInterval = setInterval(() => {
            if (this.currentState === 'dead' || ['eating', 'sleeping', 'playing', 'bathing'].includes(this.currentState)) {
                return;
            }
            ...
            if(this.context.hunger > 60 || this.context.energy < 40 || this.context.cleanliness < 40) {
                this.decreaseMood(2 + Math.floor(Math.random()*2));
            }
            
            if (this.isHungerCritical()) {
                this.decreaseLife(8 + Math.floor(Math.random()*5));
                this.showMessage('Monyet sangat kelaparan dan tubuhnya melemah!', 'error');
            }
            ...
            if (this.currentState === 'idle') {
                ...
                } else if (this.isEnergyLow() && !this.isEnergyCriticallyLow() && this.context.life > 0) {
                    this.decreaseMood(5);
                    this.transitionTo('sleeping');
    ...
    showGameOver() {
        ...
        this.monkey.className = 'monkey dead';
